...
public function student_upcoming_test_list($class_id,$section_id)
{
    ...
    //$sql = $this->db->select("s.*,sd.chapter_name,c.class_name,f.subject_name", FALSE);
    $sql = $this->db->select("s.id,s.quiz_subject_id,s.quiz_topic,s.quiz_chapter_id,s.quiz_time,s.start_date,s.end_date,td.topic_name,sd.chapter_name,f.subject_name", FALSE);
    $sql=$this->db->from('quiz_master s');
    $sql->join("sr_chapter_detail sd", "s.quiz_chapter_id = sd.chapter_id", "left");
    ...
    $sql->join("sr_subject f", "s.quiz_subject_id = f.id", "left");
    $sql->where("FIND_IN_SET( '$section_id' , s.quiz_section_id) ");
    $sql->where(array("s.is_deleted !=" => "1", 's.quiz_class_id' => $class_id, 's.quiz_type' => '3','s.start_date>'=>date('y-m-d')));
    $query = $sql->get()->result();
    return $query;
}
...
{
    $sql = $this->db->select("cd.chapter_name,td.topic_name,td.video_url,td.video_name", FALSE);
    //$sql=$this->db->from('quiz_master s');
    //$sql->join("sr_topic_detail td", "s.quiz_topic = td.topic_id", "left");
...
public function ques_ans_student_quiz_play($quiz_id,$student_id)
{
...
public function video_view_list($topic_id) {
    $this->db->select('std.video_url,std.video_name');
