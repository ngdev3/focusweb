    ...
    public function login_post() {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if ($apikey == APIKEY) {
                ...
                $query->class_name = $this->Webapi_model->class_details($query->class_id)->class_name;
    ...
    public function change_pwd_post() {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if ($apikey == APIKEY) {
                ...
                activity_logs($module, $action, $description, $id);
...
//******************************For multiple Choice Questions Starts************************************************//
            ...
            if ($_POST['fill_answer']) {
                ...
                foreach (array_filter($_POST['fill_answer']) as $question_fill => $fill_ans) {
                    ...
                    if ($wright_answer_fill_text == $student_answer_fill) {
            ...
            if ($_POST['rqsfill_answer']) {
                ...
                foreach (array_filter($_POST['rqsfill_answer']) as $question_rqsfill => $rqsfill_ans) {
                    ...
                    if ($wright_answer_rqsfill_text == $student_answer_rqsfill) {
            ...
            if ($_POST['tqsfill_answer']) {
                ...
                foreach (array_filter($_POST['tqsfill_answer']) as $question_tqsfill => $tqsfill_ans) {
                    ...
                    if ($wright_answer_tqsfill_text == $student_answer_tqsfill) {
            ...
            if ($_POST['match_question_option']) {
                ...
                $i = 0;
                foreach ($_POST['match_question_option'] as $match_question => $match_options) {
...
//                    }
                    ...
                    $match_answer_order = $_POST['match_question_answer'][$match_question];

                    //Check the user answer order   
                    $match_question_user_answer = $_POST['match_question_user_answer'][$match_question];
                    $match_question_answer_position = $_POST['match_question_answer_position'][$match_question];
                    ...
                    $ins_match['match_question_answer'] = implode(',', array_keys($match_answer_order));
...
//             die;
    ...
    public function student_upcoming_test_post() {
        ...
        if (isPostBack()) {
            ...
            if ($this->form_validation->run()) {
                if ($apikey == APIKEY) {
                    $upcoming_test = $this->Webapi_model->student_upcoming_test_list($class_id, $section_id);
<<<<<<< .mine
                   // pr($upcoming_test);
||||||| .r7933
                    pr($upcoming_test);
                ...
                if ($apikey == APIKEY) {
                    ...
                    foreach ($popular_videos as $key => $vals) {
                        ...
                        if ($popular_videos[$key]->video_url == '' && $popular_videos[$key]->video_url == null && $popular_videos[$key]->video_name != null) {
                            $popular_videos[$key]->video_name = base_url() . "uploads/profile_image/" . $vals->video_name;
                        } else if ($popular_videos[$key]->video_name == '' && $popular_videos[$key]->video_name == null && $popular_videos[$key]->video_url != null) {
                            $popular_videos[$key]->video_url = $vals->video_url;
                        } else {
                    ...
                    $success = array('ErrorCode' => 0, "message" => "Popular Videos", 'data' => $popular_videos);
...
public function ques_ans_result_view_post() {
    ...
    if (isPostBack()) {
        ...
        if ($this->form_validation->run()) {
            if ($apikey == APIKEY) {
                $result_view = $this->Webapi_model->ques_ans_result_view_list($quiz_id,$student_id);
                $student_quiz_play = $this->Webapi_model->ques_ans_student_quiz_play($quiz_id,$student_id);
               ...
               // pr($questioncontent_options_id_arr);
                ...
                pr($student_quiz_play);
                //$success = array('ErrorCode' => 0, "message" => "Result View", 'data' =>array($result_view,$student_quiz_play));
