...
class Mastervideo_model extends CI_Model {
    ...
    function add() {
        ...
        // die;
        $this->db->insert("f_self_mastery", $ins);
    ...
    function delete($id) {
        $upd['is_deleted'] = "1";
        $whr['id'] = $id;
        $this->db->update("f_self_mastery", $upd, $whr);
        //echo $this->db->last_query();die;
    ...
    function delete_multiple($id) {
        ...
        foreach ($id as $vals) {
            $upd['status'] = "delete";
            $whr['id'] = $vals;
            $abc = $this->db->update("f_self_mastery", $upd, $whr);
        }
        if ($abc) {
            return true;
        } else {
...
//Manage Service Quotes
    ...
    function list_items_service_quote_ajax() {
        ...
        $sql = $this->db->select("psq.*,concat(cu.fname,' ',cu.lname) as serviceperson_name,pl.client_name,pl.location as complaint_location", FALSE)
                ...
                ->join("f_self_mastery cu", "cu.id = psq.service_person_id", "left")
