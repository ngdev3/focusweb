...
class Morning_model extends CI_Model {
    ...
    function list_items_ajax() {
        ...
        if (!empty($requestData['search']['value'])) {
            ...
            $sql->or_where("cu.from_date LIKE '%$ser%'");
            $sql->or_where("cu.to_date LIKE '%$ser%'");
            $sql->or_where("cu.quotation_title LIKE '%$ser%'");
            $sql->or_where("cu.quotation LIKE '%$ser%'");
            $sql->or_where("cu.set_time LIKE '%$ser%'");
        ...
        foreach ($query as $i => $row) {
            ...
            $nestedData[] =  ucwords($row->set_time);
            $nestedData[] =  ucwords($row->quotation_title);

            $nestedData[] = $row->quotation;
            ...
            $nestedData[] = $this->load->view("morning/_action", array("data" => $row), true);
            $data[] = $nestedData;
        }
        $json_data = array(
    ...
    function add() {
        extract($_POST);
        // pr($_POST); 
        $ins['set_time'] = $set_time;
        $globalformate = date("H:i:s",strtotime($set_time));
        $localformate = date("H:i:s A",strtotime($set_time));
        
        $fromdates = date("Y-m-d",strtotime($fromdate));
        $todates = date("Y-m-d",strtotime($todate));
        $ins['globalformate'] = $globalformate;
        $ins['localformate'] = $localformate;
        $ins['from_date'] = $fromdates;
        $ins['to_date'] = $todates;
        $ins['quotation_title'] = $quotation_title;
        ...
        // die;
        $this->db->insert("f_morning_focus", $ins);
    ...
    function edit($id = NULL) {
        ...
        $whr['id'] = $id;
        $this->db->update("f_morning_focus", $ins, $whr);
    }

    function viewData($id) {
        $res = $this->db->select("
        cu.*, 
        cu.from_date as fromdate, 
        cu.to_date as todate,
        cu.set_time as set_time,
        ")
                ->from("f_morning_focus cu")
    ...
    function delete($id) {
        $upd['is_deleted'] = "1";
        $whr['id'] = $id;
        $this->db->update("f_morning_focus", $upd, $whr);
        //echo $this->db->last_query();die;
    ...
    function delete_multiple($id) {
        ...
        foreach ($id as $vals) {
            $upd['status'] = "delete";
            $whr['id'] = $vals;
            $abc = $this->db->update("f_morning_focus", $upd, $whr);
        }
        if ($abc) {
            return true;
        } else {
    ...
    function list_items_sales_quote_ajax() {
        ...
        $sql = $this->db->select("psq.*,concat(cu.fname,' ',cu.lname) as salesperson_name,pl.client_name,pl.location as lead_location,pr.name as product_name"
                ...
                ->join("f_morning_focus cu", "cu.id = psq.sales_person_id", "left")
...
//Manage Service Quotes
    ...
    function list_items_service_quote_ajax() {
        ...
        $sql = $this->db->select("psq.*,concat(cu.fname,' ',cu.lname) as serviceperson_name,pl.client_name,pl.location as complaint_location", FALSE)
                ...
                ->join("f_morning_focus cu", "cu.id = psq.service_person_id", "left")
