...
class Users_model extends CI_Model {
    ...
    function list_items_ajax() {
        ...
        foreach ($query as $i => $row) {
            ...
            $nestedData[] =  ucwords($row->fname);
            $nestedData[] =  ucwords($row->lname);
            $nestedData[] =  ucwords($row->mobile_no);
...
// pr($this->get_cat('1')); die;
    ...
    //    pr($_POST); die;
        ...
        $ins['lname'] = $lname;
        $ins['email'] = $email;
        $ins['mobile_no'] = $mobile;
        if($type == '2'){
            $ins['is_coach'] = 0;
            $ins['user_type'] = 2;
            $updlog['became'] = "is_add_user";

        }else if($type == '3'){
            $ins['is_coach'] = 1;
            $ins['user_type'] = 3;
            $updlog['became'] = "is_add_coach";
        ...
        $updlog['whois'] = $this->db->insert_id();
    ...
    function viewData($id) {
        $res = $this->db->select("cu.*, cu.mobile_no as mobile, cu.user_type as gender")
                ->from("users cu")
    ...
    function list_items_sales_quote_ajax() {
        ...
        $sql = $this->db->select("psq.*,concat(cu.fname,' ',cu.lname) as salesperson_name,pl.client_name,pl.location as lead_location,pr.name as product_name"
                ...
                ->join("users cu", "cu.id = psq.sales_person_id", "left")
...
//Manage Service Quotes
    ...
    function list_items_service_quote_ajax() {
        ...
        $sql = $this->db->select("psq.*,concat(cu.fname,' ',cu.lname) as serviceperson_name,pl.client_name,pl.location as complaint_location", FALSE)
                ...
                ->join("users cu", "cu.id = psq.service_person_id", "left")
