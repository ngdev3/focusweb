...
class Leadershipvideo_model extends CI_Model {
    ...
    function list_items_ajax() {
        ...
        foreach ($query as $i => $row) {
            ...
            $nestedData[] =  substr(ucwords($row->url),0,30);
    ...
    function add() {
        ...
        // die;
        $this->db->insert("f_leadership", $ins);
    ...
    function edit($id = NULL) {
        ...
        $whr['id'] = $id;
        $this->db->update("f_leadership", $ins, $whr);
    }
    ...
    function viewData($id) {
        ...
        ")
                ->from("f_leadership cu")
                ->where(array("cu.id" => $id,"cu.type" => '2'))
                ->get()
                ->row();
        return $res;
    }
    ...
    function delete($id) {
        $upd['is_deleted'] = "1";
        $whr['id'] = $id;
        $this->db->update("f_leadership", $upd, $whr);
        //echo $this->db->last_query();die;
    ...
    function delete_multiple($id) {
        ...
        foreach ($id as $vals) {
            $upd['status'] = "delete";
            $whr['id'] = $vals;
            $abc = $this->db->update("f_leadership", $upd, $whr);
        }
        if ($abc) {
            return true;
        } else {
    ...
    function list_items_sales_quote_ajax() {
        ...
        $sql = $this->db->select("psq.*,concat(cu.fname,' ',cu.lname) as salesperson_name,pl.client_name,pl.location as lead_location,pr.name as product_name"
                ...
                ->join("f_leadership cu", "cu.id = psq.sales_person_id", "left")
...
//Manage Service Quotes
    ...
    function list_items_service_quote_ajax() {
        ...
        $sql = $this->db->select("psq.*,concat(cu.fname,' ',cu.lname) as serviceperson_name,pl.client_name,pl.location as complaint_location", FALSE)
                ...
                ->join("f_leadership cu", "cu.id = psq.service_person_id", "left")
