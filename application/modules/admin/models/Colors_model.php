...
class Colors_model extends CI_Model {
    ...
    function list_items_ajax() {
        ...
        foreach ($query as $i => $row) {
            ...
            $full_name = ucwords($row->fname);
            $nestedData[] =  ucwords($row->scheme_color_name);
            $nestedData[] =  ucwords($row->background_color);
            $nestedData[] =  ucwords($row->font_color);

            $nestedData[] = $row->button_color;
            ...
            $nestedData[] = $this->load->view("color/_action", array("data" => $row), true);
            $data[] = $nestedData;
        }
        $json_data = array(
    ...
    function add() {
        extract($_POST);
        $ins['scheme_color_name'] = $colorname;
        $ins['background_color'] = $background_name;
        $ins['font_color'] = $font_color;
        $ins['button_color'] = $button_color;
        $ins['status'] = $status;
        $ins['added_by'] = getUserInfos()->id;
        $ins['created_date'] = current_datetime();
        $this->db->insert("f_color_schemes", $ins);
    ...
    function edit($id = NULL) {
        ...
        $whr['id'] = $id;
        $this->db->update("f_color_schemes", $ins, $whr);
    }

    function viewData($id) {
        $res = $this->db->select("
        cu.*, 
        cu.scheme_color_name as colorname, 
        cu.background_color as background_name,
        cu.font_color as font_color,
        cu.button_color as button_color,
    ...
    function delete($id) {
        $upd['is_deleted'] = "1";
        $whr['id'] = $id;
        $this->db->update("f_color_schemes", $upd, $whr);
        //echo $this->db->last_query();die;
    ...
    function delete_multiple($id) {
        ...
        foreach ($id as $vals) {
            $upd['status'] = "delete";
            $whr['id'] = $vals;
            $abc = $this->db->update("f_color_schemes", $upd, $whr);
        }
        if ($abc) {
            return true;
        } else {
    ...
    function list_items_sales_quote_ajax() {
        ...
        $sql = $this->db->select("psq.*,concat(cu.fname,' ',cu.lname) as salesperson_name,pl.client_name,pl.location as lead_location,pr.name as product_name"
                ...
                ->join("f_color_schemes cu", "cu.id = psq.sales_person_id", "left")
...
//Manage Service Quotes
    ...
    function list_items_service_quote_ajax() {
        ...
        $sql = $this->db->select("psq.*,concat(cu.fname,' ',cu.lname) as serviceperson_name,pl.client_name,pl.location as complaint_location", FALSE)
                ...
                ->join("f_color_schemes cu", "cu.id = psq.service_person_id", "left")
