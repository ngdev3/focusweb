...
class Cms extends MY_Controller {
    ...
    function listing() {
        ...
        $data['title'] = "CMS";
        $data['page_title'] = "CMS";
        $data['page'] = "cms/list_items";
        $data['breadcrumb'] = array("Dashboard" => base_url('dashboard/'), "CMS" => base_url('admin/cms/listing'));
        _layout($data);
    }

    function list_items_ajax() {
        $res = $this->cms_model->list_items_ajax();
    ...
    function add() {
    //  pr($_POST); die;
        $data['page_title'] = "Add cms Focus";
        $data['page'] = "cms/add";
        $data['title'] = "Add cms Focus";
        $data['breadcrumb'] = array("Dashboard" => base_url(), "cms Focus" => base_url('admin/cms/listing'), "Add cms Focus" => base_url('admin/cms/add'));
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $last_id = $this->cms_model->add();
            $this->session->set_flashdata("alert", array("c" => "s", "m" => "cms Added Successfully. "));
    ...
    public function edit() {
        ...
        $data['res'] = $this->cms_model->viewData($id);
        // pr($data['res']);
        // $this->form_validation->set_rules('type', 'Type', 'trim|required');
        $this->form_validation->set_rules('description', 'Description', 'trim|required');
        $this->form_validation->set_rules('status', 'Status', 'trim|required');
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $this->session->set_flashdata("alert", array("c" => "s", "m" => "cms Updated Successfully. "));
            redirect('admin/cms/listing');
        }
   //     $data['roles'] = $this->cms_model->get_backendteam();
      //  $data['location'] = get_where("pr_location", array("status" => "1", "is_deleted" => "0"));

        $data['title'] = "Edit CMS Focus";
        $data['page_title'] = "Edit CMS Focus";
        $data['page'] = "cms/add";
        $data['breadcrumb'] = array("Dashboard" => base_url(), "CMS Focus" => base_url('admin/cms/listing'), "Edit CMS Focus" => "");
        _layout($data);
    }
        ...
        $id = ID_decode($id);
        if (!empty($id)) {
            $data['res'] = $this->cms_model->view($id);
            
            //pr($data);die;
            $data['title'] = 'View cms Focus';
            $data['page_title'] = 'View cms Focus';
            $data['breadcrumb'] = array("Dashboard" => base_url('dashboard/'), "cms Focus List" => base_url('admin/cms/list_items'), "View cms Focus" => "");
    ...
    function becomecoach() {
        $id = $_POST['id'];
        $res = $this->cms_model->assign_coach($id);
        if ($res['status'] != "false") {
    ...
    function becomeuser() {
        $id = $_POST['id'];
        $res = $this->cms_model->assign_user($id);
        if ($res['status'] != "false") {
    ...
    public function delete_multiple() {
        ...
        $res = $this->cms_model->delete_multiple($id);
        if ($res == "true") {
            $this->session->set_flashdata("alert", array("c" => "s", "m" => "User Deleted Successfully. "));
            echo json_encode($res);
        } else {
    ...
    function upload_portfolio_image($files, $user_id) {
        ...
        if (isset($portfolioimage) && !empty($portfolioimage)) {
            $this->cms_model->save_image($portfolioimage, $user_id);
        }
    }
    ...
    function list_items_sales_quote() {
        ...
        $data['quote_age'] = $this->cms_model->get_quote_age();

        $data['title'] = "Recieved Sales Quote";
        $data['page_title'] = "Recieved Sales Quote";
        $data['page'] = "list_items_sales_quote";
        $data['breadcrumb'] = array("Dashboard" => base_url('dashboard/'), "Recieved Sales Quote" => base_url('admin/cms/list_items_sales_quote'));
        _layout($data);
    }

    function list_items_sales_quote_ajax() {
        $res = $this->cms_model->list_items_sales_quote_ajax();
    ...
    public function view_sales_quote($id) {
        ...
        if (!empty($id)) {
            $data['res'] = $this->cms_model->view_sales_quote($id);
            $data['res']->rate=$this->cms_model->getsales_quote_rate($id);
           // pr($data);
            $data['title'] = 'View Sales Quote';
            $data['page_title'] = 'View Sales Quote';
            $data['breadcrumb'] = array("Dashboard" => base_url('dashboard/'), "Recieved Sales Quote" => base_url('admin/cms/list_items_sales_quote'), "View Sales Quote" => "");
        ...
        if (!empty($id)) {
            ...
            if(isPostBack()){
                ...
                if($this->form_validation->run() == TRUE) {
                    ...
                    $last_id = $this->cms_model->sales_quote_details($id);
                   ...
                   //echo "done";die;
                    ...
                    redirect("admin/cms/list_items_sales_quote");
                }
            ...
            $data['breadcrumb'] = array("Dashboard" => base_url('dashboard/'), "Recieved Sales Quote" => base_url('admin/cms/list_items_sales_quote'), "Send Sales Quote" => "");
    ...
    function list_items_service_quote() {
        ...
        $data['breadcrumb'] = array("Dashboard" => base_url('dashboard/'), "Recieved Service Quote" => base_url('admin/cms/list_items_service_quote'));
        _layout($data);
    }

    function list_items_service_quote_ajax() {
        $res = $this->cms_model->list_items_service_quote_ajax();
     ...
     public function view_service_quote($id) {
        ...
        if (!empty($id)) {
            $data['res'] = $this->cms_model->view_service_quote($id);
            $data['product']=$this->cms_model->view_service_quote_product($id);
            ...
            $data['breadcrumb'] = array("Dashboard" => base_url('dashboard/'), "Recieved Service Quote" => base_url('admin/cms/list_items_service_quote'), "View Service Quote" => "");
    ...
    public function send_service_quote($id) {
        ...
        if (!empty($id)) {
            ...
            if(isPostBack()){
                ...
                if($_FILES) {
                    $last_id = $this->cms_model->service_quote_details($id);
                    if (isset($_FILES['specifications'])) {
                        $this->upload_specifications($_FILES['specifications']);
                    }
                   //echo "done";die;
                    $this->session->set_flashdata("alert", array("c" => "s", "m" => "Service Quote Details Added Successfully. "));
                    redirect("admin/cms/list_items_service_quote");
                }
            ...
            $data['breadcrumb'] = array("Dashboard" => base_url('dashboard/'), "Recieved Service Quote" => base_url('admin/cms/list_items_service_quote'), "Send Service Quote" => "");
