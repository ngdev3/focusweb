...
class Users extends MY_Controller {
    ...
    function listing() {
        ...
        $data['page'] = "users/list_items";
        $data['breadcrumb'] = array("Dashboard" => base_url('dashboard/'), "User List" => base_url('admin/users/listing'));
        _layout($data);
    }
    ...
    function add() {
        ...
        $data['page'] = "users/add";
        $data['title'] = "Add User";
        $data['breadcrumb'] = array("Dashboard" => base_url(), "User List" => base_url('admin/users/listing'), "Add User" => base_url('admin/users/add'));
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            redirect("admin/users/listing" . $_POST['role']);
        }
        _layout($data);
    }
   ...
   //     $data['roles'] = $this->users_model->get_backendteam();
      //  $data['location'] = get_where("pr_location", array("status" => "1", "is_deleted" => "0"));

        $data['title'] = "Edit User Team";
        $data['page_title'] = "Edit User Team";
        $data['page'] = "users/add";
        $data['breadcrumb'] = array("Dashboard" => base_url(), "User List" => base_url('admin/users/listing'), "Edit User Team" => "");
        _layout($data);
    }
        ...
        $id = ID_decode($id);
        if (!empty($id)) {
            $data['res'] = $this->users_model->view($id);
            
            //pr($data);die;
            $data['title'] = 'View User';
            $data['page_title'] = 'View User';
            $data['breadcrumb'] = array("Dashboard" => base_url('dashboard/'), "User List" => base_url('admin/users/list_items'), "View User" => "");
    ...
    function becomecoach() {
      //  pr(($_POST['data'])); die;
        ...
        $id = $_POST['id'];
        $res = $this->users_model->assign_coach($id);
        if ($res['status'] != "false") {
    ...
    public function delete_multiple() {
        ...
        $res = $this->users_model->delete_multiple($id);
        if ($res == "true") {
            $this->session->set_flashdata("alert", array("c" => "s", "m" => "User Deleted Successfully. "));
            echo json_encode($res);
        } else {
    ...
    function upload_portfolio_image($files, $user_id) {
        ...
        if (isset($portfolioimage) && !empty($portfolioimage)) {
            $this->users_model->save_image($portfolioimage, $user_id);
        }
    }
    ...
    function list_items_sales_quote() {
        ...
        $data['quote_age'] = $this->users_model->get_quote_age();

        $data['title'] = "Recieved Sales Quote";
        $data['page_title'] = "Recieved Sales Quote";
        $data['page'] = "list_items_sales_quote";
        $data['breadcrumb'] = array("Dashboard" => base_url('dashboard/'), "Recieved Sales Quote" => base_url('admin/users/list_items_sales_quote'));
        _layout($data);
    }

    function list_items_sales_quote_ajax() {
        $res = $this->users_model->list_items_sales_quote_ajax();
    ...
    public function view_sales_quote($id) {
        ...
        if (!empty($id)) {
            $data['res'] = $this->users_model->view_sales_quote($id);
            $data['res']->rate=$this->users_model->getsales_quote_rate($id);
           // pr($data);
            $data['title'] = 'View Sales Quote';
            $data['page_title'] = 'View Sales Quote';
            $data['breadcrumb'] = array("Dashboard" => base_url('dashboard/'), "Recieved Sales Quote" => base_url('admin/users/list_items_sales_quote'), "View Sales Quote" => "");
        ...
        if (!empty($id)) {
            ...
            if(isPostBack()){
                ...
                if($this->form_validation->run() == TRUE) {
                    ...
                    $last_id = $this->users_model->sales_quote_details($id);
                   ...
                   //echo "done";die;
                    ...
                    redirect("admin/users/list_items_sales_quote");
                }
            ...
            $data['breadcrumb'] = array("Dashboard" => base_url('dashboard/'), "Recieved Sales Quote" => base_url('admin/users/list_items_sales_quote'), "Send Sales Quote" => "");
    ...
    function list_items_service_quote() {
        ...
        $data['breadcrumb'] = array("Dashboard" => base_url('dashboard/'), "Recieved Service Quote" => base_url('admin/users/list_items_service_quote'));
        _layout($data);
    }

    function list_items_service_quote_ajax() {
        $res = $this->users_model->list_items_service_quote_ajax();
     ...
     public function view_service_quote($id) {
        ...
        if (!empty($id)) {
            $data['res'] = $this->users_model->view_service_quote($id);
            $data['product']=$this->users_model->view_service_quote_product($id);
            ...
            $data['breadcrumb'] = array("Dashboard" => base_url('dashboard/'), "Recieved Service Quote" => base_url('admin/users/list_items_service_quote'), "View Service Quote" => "");
    ...
    public function send_service_quote($id) {
        ...
        if (!empty($id)) {
            ...
            if(isPostBack()){
                ...
                if($_FILES) {
                    $last_id = $this->users_model->service_quote_details($id);
                    if (isset($_FILES['specifications'])) {
                        $this->upload_specifications($_FILES['specifications']);
                    }
                   //echo "done";die;
                    $this->session->set_flashdata("alert", array("c" => "s", "m" => "Service Quote Details Added Successfully. "));
                    redirect("admin/users/list_items_service_quote");
                }
            ...
            $data['breadcrumb'] = array("Dashboard" => base_url('dashboard/'), "Recieved Service Quote" => base_url('admin/users/list_items_service_quote'), "Send Service Quote" => "");
