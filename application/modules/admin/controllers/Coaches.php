...
class Coaches extends MY_Controller {
    ...
    function listing() {
        // die;
        $data['title'] = "Coaches Center";
        $data['page_title'] = "Coaches Center";
        $data['page'] = "coaches/list_items";
        $data['breadcrumb'] = array("Dashboard" => base_url('dashboard/'), "Coaches Center" => base_url('admin/coaches/listing'));
        _layout($data);
    }

    function list_items_ajax() {
        $res = $this->coaches_model->list_items_ajax();
    ...
    function add() {
    //  pr($_POST); die;
        $data['page_title'] = "Add Coaches Center";
        $data['page'] = "coaches/add";
        $data['title'] = "Add Coaches Center";
        $data['breadcrumb'] = array("Dashboard" => base_url(), "Coaches Center" => base_url('admin/coaches/listing'), "Add Coaches Center" => base_url('admin/coaches/add'));
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $last_id = $this->coaches_model->add();
            $this->session->set_flashdata("alert", array("c" => "s", "m" => "Coaches Center Added Successfully. "));
            redirect("admin/coaches/listing");
        }
        _layout($data);
    }
    ...
    public function edit() {
        ...
        $data['res'] = $this->coaches_model->viewData($id);
        $this->form_validation->set_rules('video_title', 'Video Ttle', "trim|required");
        $this->form_validation->set_rules('video_url', 'Video Url', "trim|required");
        $this->form_validation->set_rules('description', 'Description', 'trim|required');
        $this->form_validation->set_rules('status', 'Status', 'trim|required');
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $this->session->set_flashdata("alert", array("c" => "s", "m" => "Coaches Center Updated Successfully. "));
            redirect('admin/coaches/listing');
        }
   //     $data['roles'] = $this->coaches_model->get_backendteam();
      //  $data['location'] = get_where("pr_location", array("status" => "1", "is_deleted" => "0"));

        $data['title'] = "Edit Coaches Center";
        $data['page_title'] = "Edit Coaches Center";
        $data['page'] = "coaches/add";
        $data['breadcrumb'] = array("Dashboard" => base_url(), "Coaches Center" => base_url('admin/coaches/listing'), "Edit Coaches Center" => "");
        _layout($data);
    }
        ...
        $id = ID_decode($id);
        if (!empty($id)) {
            $data['res'] = $this->coaches_model->view($id);
            
            //pr($data);die;
            $data['title'] = 'View Coaches Center';
            $data['page_title'] = 'View Coaches Center';
            $data['breadcrumb'] = array("Dashboard" => base_url('dashboard/'), "Coaches Center List" => base_url('admin/coaches/list_items'), "View Coaches Center" => "");
          
            $data['page'] = "coaches/view";
            _layout($data);
        }
    }
    ...
    function delete() {
        $id = $_POST['id'];
        $res = $this->coaches_model->delete($id);
        if ($res['status'] != "false") {
    ...
    function becomecoach() {
        $id = $_POST['id'];
        $res = $this->coaches_model->assign_coach($id);
        if ($res['status'] != "false") {
    ...
    function becomeuser() {
        $id = $_POST['id'];
        $res = $this->coaches_model->assign_user($id);
        if ($res['status'] != "false") {
    ...
    public function delete_multiple() {
        ...
        $res = $this->coaches_model->delete_multiple($id);
        if ($res == "true") {
            $this->session->set_flashdata("alert", array("c" => "s", "m" => "User Deleted Successfully. "));
            echo json_encode($res);
        } else {
    ...
    function upload_portfolio_image($files, $user_id) {
        ...
        if (isset($portfolioimage) && !empty($portfolioimage)) {
            $this->coaches_model->save_image($portfolioimage, $user_id);
        }
    }
    ...
    function list_items_sales_quote() {
        ...
        $data['quote_age'] = $this->coaches_model->get_quote_age();

        $data['title'] = "Recieved Sales Quote";
        $data['page_title'] = "Recieved Sales Quote";
        $data['page'] = "list_items_sales_quote";
        $data['breadcrumb'] = array("Dashboard" => base_url('dashboard/'), "Recieved Sales Quote" => base_url('admin/coaches/list_items_sales_quote'));
        _layout($data);
    }

    function list_items_sales_quote_ajax() {
        $res = $this->coaches_model->list_items_sales_quote_ajax();
    ...
    public function view_sales_quote($id) {
        ...
        if (!empty($id)) {
            $data['res'] = $this->coaches_model->view_sales_quote($id);
            $data['res']->rate=$this->coaches_model->getsales_quote_rate($id);
           // pr($data);
            $data['title'] = 'View Sales Quote';
            $data['page_title'] = 'View Sales Quote';
            $data['breadcrumb'] = array("Dashboard" => base_url('dashboard/'), "Recieved Sales Quote" => base_url('admin/coaches/list_items_sales_quote'), "View Sales Quote" => "");
        ...
        if (!empty($id)) {
            ...
            if(isPostBack()){
                ...
                if($this->form_validation->run() == TRUE) {
                    ...
                    $last_id = $this->coaches_model->sales_quote_details($id);
                   ...
                   //echo "done";die;
                    ...
                    redirect("admin/coaches/list_items_sales_quote");
                }
            ...
            $data['breadcrumb'] = array("Dashboard" => base_url('dashboard/'), "Recieved Sales Quote" => base_url('admin/coaches/list_items_sales_quote'), "Send Sales Quote" => "");
    ...
    function list_items_service_quote() {
        ...
        $data['breadcrumb'] = array("Dashboard" => base_url('dashboard/'), "Recieved Service Quote" => base_url('admin/coaches/list_items_service_quote'));
        _layout($data);
    }

    function list_items_service_quote_ajax() {
        $res = $this->coaches_model->list_items_service_quote_ajax();
     ...
     public function view_service_quote($id) {
        ...
        if (!empty($id)) {
            $data['res'] = $this->coaches_model->view_service_quote($id);
            $data['product']=$this->coaches_model->view_service_quote_product($id);
            ...
            $data['breadcrumb'] = array("Dashboard" => base_url('dashboard/'), "Recieved Service Quote" => base_url('admin/coaches/list_items_service_quote'), "View Service Quote" => "");
    ...
    public function send_service_quote($id) {
        ...
        if (!empty($id)) {
            ...
            if(isPostBack()){
                ...
                if($_FILES) {
                    $last_id = $this->coaches_model->service_quote_details($id);
                    if (isset($_FILES['specifications'])) {
                        $this->upload_specifications($_FILES['specifications']);
                    }
                   //echo "done";die;
                    $this->session->set_flashdata("alert", array("c" => "s", "m" => "Service Quote Details Added Successfully. "));
                    redirect("admin/coaches/list_items_service_quote");
                }
            ...
            $data['breadcrumb'] = array("Dashboard" => base_url('dashboard/'), "Recieved Service Quote" => base_url('admin/coaches/list_items_service_quote'), "Send Service Quote" => "");
