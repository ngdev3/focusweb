...
class Manage_service_order extends MY_Controller {

    public function __construct() {
        parent::__construct();
        $this->load->model("manage_service_order_model");
        $this->load->model("manage_sales_order/manage_sales_order_model");
        $this->load->model("salesperson/salesperson_model");
        $this->load->model("serviceperson/serviceperson_model");
        $this->load->model("complaint/Complaint_model");
        
    }

    public function index() {
        $this->load->view('serviceperson_view');
    }
    ...
    function list_items() {
        ...
        $data['serviceperson'] = $this->manage_service_order_model->getserviceperson_filter();
        $data['manager'] = $this->serviceperson_model->get_manager_user();

        $data['coordinator']=$this->manage_service_order_model->getallservicecordinator();
        $data['title'] = "List Of Order (Service)";
        $data['page_title'] = "List Of Order (Service)";
        $data['page'] = "list_items";
        $data['breadcrumb'] = array("Dashboard" => base_url('dashboard/'), "List Of Order (Service)" => base_url('manage_service_order/list_items'));
        _layout($data);
    }

    function list_items_ajax() {
        $res = $this->manage_service_order_model->list_items_ajax();
    ...
    function add() {
        ...
        $data['manager'] = $this->salesperson_model->get_manager_user();
        $data['serviceperson'] = $this->manage_service_order_model->getserviceperson_filter();
       // $data['cordinator']  = $this->manage_sales_order_model->getservicecordinator($data['res']->manager_id);
        //$data['client']= $this->manage_service_order_model->getclient();
        $data['product']= $this->manage_service_order_model->getproduct();
        $data['payment_mode']= $this->manage_service_order_model->getpayment_type();
        
        $data['page_title'] = "Create Order";
        $data['page'] = "add";
        $data['title'] = "Create Order";

        $ins_id = $this->manage_service_order_model->last_id_get();
        $ins_ids = $ins_id->id + 1;
        $data['order_id'] = "OD$ins_ids";

        $data['breadcrumb'] = array("Home" => base_url(), "List Of Order (Service)" => base_url('manage_service_order/list_items'), "Create Order" => base_url('manage_service_order/add'));
        ...
        // pr($_POST);die;
         ...
         $this->form_validation->set_rules('serviceperson_id', 'Service Person Name', "trim|required");
        ...
        if ($this->form_validation->run() == TRUE) {
            //echo"chusoooo";
//pr($_POST);die;
            $last_id = $this->manage_service_order_model->add();
            $this->session->set_flashdata("alert", array("c" => "s", "m" => "Order(service) Created Successfully. "));
            redirect("manage_service_order/list_items");
        }
    ...
    public function view($id) {
        ...
        if (!empty($id)) {
            $data['res'] = $this->manage_service_order_model->view($id);
            $data['detail'] = $this->manage_service_order_model->detail($id);
            
            //pr($data);die;
            $data['title'] = 'View Order';
            $data['page_title'] = 'View Order';
            $data['breadcrumb'] = array("Dashboard" => base_url('dashboard/'), "List Of Order (Service)" => base_url('manage_service_order/list_items'), "View Order" => "");
            //pr($data);die;
            $data['page'] = "view";
            _layout($data);
        }
...
public function get_product_details()
{
 ...
 public function getservicecordinator() {
    // $class_id = $_POST['class_id'];
     $manager_id = $_POST['manager_id'];
     $subData = $this->manage_service_order_model->getservicecordinator($manager_id);
     $output = '';
     if (!empty($subData)) {
 ...
 public function getserviceperson() {
    $cordinator_id = $_POST['cordinator_id'];
    $manager_id = $_POST['manager_id'];
    $subData = $this->manage_service_order_model->getserviceperson($manager_id,$cordinator_id);
    $output = '';
    if (!empty($subData)) {
 ...
 public function getclientperson() {
        $cordinator_id = $_POST['cordinator_id'];
        $manager_id = $_POST['manager_id'];
        $serviceperson_id = $_POST['serviceperson_id'];
        $subData = $this->manage_service_order_model->getclientperson($manager_id,$cordinator_id,$serviceperson_id);
        //$subData = $this->manage_service_order_model->getclient();
        $output = '';
        if (!empty($subData)) {
            $output .= '<op1:05 PM 12/28/2018tion value=""> Select Client</option>';
            foreach ($subData as $type) {
                $output .= '<option value="' . $type->complaint_id . '">' . ucwords($type->client_name) . '</option>';
            }
        } else {
            $output .= '<option value=""> No Client Available</option>';
        }
